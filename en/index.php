<?php require_once($_SERVER['DOCUMENT_ROOT'].'/en/inc/dochead.php'); ?>
<link rel="stylesheet" type="text/css" href="//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.css"/>
<link rel="stylesheet" href="/en/assets/css/main.css">
</head>
<body>
	<?php require_once($_SERVER['DOCUMENT_ROOT'].'/en/inc/header.php'); ?>
    <?php require_once($_SERVER['DOCUMENT_ROOT'].'/en/inc/aside.php'); ?>
    
	<!-- Content -->
    <main id="content">
    	<!-- jumbotrons slider -->
    	<div class="jumbotron">
			<div class="container-fluid">
				<div class="row">
					<div class="col-12 col-xl-8 restrict">
						<ul>
							<li class="visual-list">
								<video class="video" loop muted autoplay>
									<source src="/en/assets/video/33480139-preview.mp4" type="video/mp4">이 브라우저는 동영상을 지원하지 않습니다.
								</video>
								<div class="summary">
									<h2>
										The best world-class Company <br>
										with the spirit which is healthy<br>
										in the body which is healthy
									</h2>
									<p>One person's dream is just a dream, The dream of having many people becomes a reality.</p>
									<div class="btn-area">
										<p>
											<a href="#" class="btn btn-default" role="button">See more</a>
										</p>
									</div>
								</div>
							</li>
							<li class="visual-list">
								<img src="/en/assets/images/img_visual_md.jpg" class="img-fluid d-sm-block d-xl-none" alt="">
								<img src="/en/assets/images/img_visual_xl.jpg" class="img-fluid d-none d-xl-block" alt="">
								<div class="summary">
									<h2>
										The best world-class Company <br>
										with the spirit which is healthy<br>
										in the body which is healthy
									</h2>
									<p>One person's dream is just a dream, The dream of having many people becomes a reality.</p>
									<div class="btn-area">
										<p>
											<a href="#" class="btn btn-default" role="button">See more</a>
										</p>
									</div>
								</div>
							</li>
							<li class="visual-list">
								<img src="/en/assets/images/img_visual_md.jpg" class="img-fluid d-sm-block d-xl-none" alt="">
								<img src="/en/assets/images/img_visual_xl.jpg" class="img-fluid d-none d-xl-block" alt="">
								<div class="summary">
									<h2>
										The best world-class Company <br>
										with the spirit which is healthy<br>
										in the body which is healthy
									</h2>
									<p>One person's dream is just a dream, The dream of having many people becomes a reality.</p>
									<div class="btn-area">
										<p>
											<a href="#" class="btn btn-default" role="button">See more</a>
										</p>
									</div>
								</div>
							</li>
						</ul>
					</div>
					<div class="col-12 col-xl-4 restrict">
						<div class="row">
							<div class="col-6 col-xl-12 expand">
								<div class="system-wrap">
									<div class="system content2">
										<a href="#">
											<div class="item">
												<img src="/en/assets/images/img_content2.png" class="img-fluid" alt="FPD Inspection System">
											</div>
											<div class="details">
												<h3>FPD Transfer<br class="d-xl-none"> System</h3>
												<p class="d-none d-xl-block">
													FPD Transfer System의 간략한 설명이
													노출되며 해당 제품군으로 바로가기 할
													수 있는 영역입니다.
												</p>
												<div class="btn-box d-none d-md-block">
													Click
												</div>
											</div>
										</a>
										<!--div class="item">
											<img src="/en/assets/images/img_content2.png" class="img-fluid" alt="FPD Inspection System">
										</div>
										<div class="details">
											<h3>FPD Transfer<br class="d-xl-none"> System</h3>
											<p class="d-none d-xl-block">
												FPD Transfer System의 간략한 설명이
												노출되며 해당 제품군으로 바로가기 할
												수 있는 영역입니다.
											</p>
											<div class="btn-area d-none d-md-block">
												<p>
													<a href="#" class="btn btn-default" role="button">바로가기</a>
												</p>
											</div>
										</div-->
									</div>
								</div>
							</div>
							<div class="col-6 col-xl-12 expand">
								<div class="system-wrap">
									<div class="system content1">
										<a href="#">
											<div class="item">
												<img src="/en/assets/images/img_content1.png" class="img-fluid" alt="FPD Transfer System">
											</div>
											<div class="details">
												<h3>FPD Inspection<br class="d-xl-none"> System</h3>
												<p class="d-none d-xl-block">
													FPD  Inspection System의 간략한
													설명이노출되며 해당 제품군으로 바로가기
													할 수 있는 영역입니다.
												</p>
												<div class="btn-box d-none d-md-block">
													Click
												</div>
											</div>
										</a>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
		<section class="news">
			<div class="container">
				<div class="section-header">
					<h3>NEWS</h3>
				</div>
				<div class="section-content">
					<ul>
						<li>
							<div class="row">
								<div class="d-none d-md-block col-2">
									<span>2018.06.01</span>
								</div>
								<div class="col-12 col-md-10">
									<a href="#">
										<h4>Renewal of website</h4>
									</a>
								</div>
							</div>
						</li>
						<li>
							<div class="row">
								<div class="d-none d-md-block col-2">
									<span>2018.06.01</span>
								</div>
								<div class="col-12 col-md-10">
									<a href="#">
										<h4>Renewal of website</h4>
									</a>
								</div>
							</div>
						</li>
						<li>
							<div class="row">
								<div class="d-none d-md-block col-2">
									<span>2018.06.01</span>
								</div>
								<div class="col-12 col-md-10">
									<a href="#">
										<h4>Renewal of website</h4>
									</a>
								</div>
							</div>
						</li>
					</ul>
				</div>
			</div>
		</section>
		<article class="quick">
			<div class="container">
				<div class="row">
					<div class="col-12 col-xl-7 order-xl-last">
						<form action="#">
							<div class="form-group">
								<label for="search" class="sr-only">검색하기</label>
								<input type="text" id="search" class="form-control" placeholder="Enter search word">
								<button type="submit" class="btn btn-search"></button>
							</div>
						</form>
						<div class="quick-menu">
							<ul class="qucik-info">
								<li>
									<a href="#">
										<h4>Q&amp;A</h4>
										<p>If you ask me any questions, I will answer them kindly.</p>
									</a>
								</li>
								<li>
									<a href="#">
										<h4>Recruiting</h4>
										<p>We are looking for a talent in our company that will realize Human resource</p>
									</a>
								</li>
							</ul>
							<ul class="qucik-list">
								<li>
									<a href="#">
										<div class="items"></div>
										<div class="info">
											<h4>Certification<br> status</h4>
											<p>Click</p>
										</div>
									</a>
								</li>
								<li>
									<a href="#">
										<div class="items"></div>
										<div class="info">
											<h4>Location</h4>
											<p>Click</p>
										</div>
									</a>
								</li>
								<li>
									<a href="#">
										<div class="items"></div>
										<div class="info">
											<h4>Notice</h4>
											<p>Click</p>
										</div>
									</a>
								</li>
							</ul>
						</div>
					</div>
					<div class="col-12 col-xl-5 order-xl-first">
						<div class="bg-utosys">
                            <figure class="d-none d-xl-block">
						        <img src="/en/assets/images/img_utosys02.jpg" class="img-fluid" alt="">
                            </figure>
                            <div class="summary">
                                <h4>
                                    The best world-class Company <br>
									with the spirit which is healthy <br>
									in the body which is healthy
                                </h4>
                                <div class="btn-area">
                                    <p>
                                        <a href="#" class="btn btn-details" role="button">See more</a>
                                    </p>
                                </div>
                            </div>
						</div>
					</div>
				</div>
			</div>
		</article>
	</main>

    <!-- main popup -->
    <div class="modal fade main-popup" id="mainModalPopup" tabindex="-1" role="dialog" aria-labelledby="mainModalPopup" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-body">
                    <ul class="modal-slide">
                        <li>
                            <a href="#">
                                <figure>
                                    <img src="/en/assets/images/bg_utosys.gif" class="img-fluid" alt="">
                                </figure>
                            </a>
                        </li>
                        <li>
                            <a href="#">
                                <figure>
                                    <img src="/en/assets/images/img_visual_md.jpg" class="img-fluid" alt="">
                                </figure>
                            </a>
                        </li>
                        <li>
                            <a href="#">
                                <figure>
                                    <img src="/en/assets/images/img_visual_xl.jpg" class="img-fluid" alt="">
                                </figure>
                            </a>
                        </li>
                    </ul>
                </div>
                <div class="modal-footer">
                    <div class="form-group">
                        <label><input type="checkbox"> 오늘 하루 열지 않기</label>
                    </div>
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">닫기</button>
                </div>
            </div>
        </div>
    </div>
    
	<?php require_once($_SERVER['DOCUMENT_ROOT'].'/en/inc/footer.php'); ?>
    <?php require_once($_SERVER['DOCUMENT_ROOT'].'/en/inc/docfoot.php'); ?>
    <script type="text/javascript" src="//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.min.js"></script>
    <script src="/en/assets/js/main.js"></script>
    <script>
		(function($){
			// main modal popup
			$('.main-popup').modal().show();
			$(window).ready(function(){
				$('.main-popup .modal-slide').slick({
					autoplay: true,
					autoplaySpeed: 5000,
					adaptiveHeight: true,
					arrows: false,
					dots: true,
					infinite: true,
				});
			});
		})(jQuery);
	</script>
</body>
</html>